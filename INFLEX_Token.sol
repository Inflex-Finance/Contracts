...
contract SafestMoon is Context, IERC20, Ownable {
    ...
    string private _symbol = "SAFESTMOON";
    ...
    event bnbSwappedForTokens(uint256 amountBNB);
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 5, "Amount must be less than or equal to 5");
        _taxFee = taxFee;
        emit TaxFeeUpdated(taxFee);
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 5, "Amount must be less than or equal to 5");
    ...
    //used to buyback and burn these tokens
    function swapBNBForTokens(uint256 amountBNB) private {
        address[] memory path = new address[](2);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        //if there is leftover BNB, then swap it back for tokens and burn them
        if(balanceBNB > 0) {
            swapBNBForTokens(balanceBNB);
            emit bnbSwappedForTokens(balanceBNB);
        }
    }
